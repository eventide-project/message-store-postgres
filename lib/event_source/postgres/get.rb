  ...
  module Postgres
    class Get
      ...
      def call(position: nil)
        logger.trace "Getting event data (Stream Position: #{position.inspect}, Stream Name: #{stream.name}, Category: #{stream.category?}, Batch Size: #{batch_size.inspect}, Precedence: #{precedence.inspect}, Partition: #{partition.inspect})"

        records = get_records(stream, position)

        events = convert(records)

        logger.debug "Finished getting event data (Count: #{events.length}, Stream Position: #{position.inspect}, Stream Name: #{stream.name}, Category: #{stream.category?}, Batch Size: #{batch_size.inspect}, Precedence: #{precedence.inspect}, Partition: #{partition.inspect})"

        events
      end

      def get_records(stream, position)
        logger.trace "Getting records (Stream: #{stream.name}, Category: #{stream.category?}, Stream Position: #{position.inspect}, Batch Size: #{batch_size.inspect}, Precedence: #{precedence.inspect}, Partition: #{partition.inspect})"
        ...
        logger.debug "Finished getting records (Count: #{records.ntuples}, Stream: #{stream.name}, Category: #{stream.category?}, Stream Position: #{position.inspect}, Batch Size: #{batch_size.inspect}, Precedence: #{precedence.inspect}, Partition: #{partition.inspect})"
